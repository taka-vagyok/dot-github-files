...
3. **オーケストレーターの制限**: オーケストレーターの `tools` には `edit` などの直接編集ツールを含めないでください。
4. **スキルの構成**: スキルは単一のファイルではなく、ディレクトリ（例：`skills/read-file-content/SKILL.md`）として構成し、複雑な処理は `scripts/` に分割してください。
